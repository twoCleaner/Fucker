...
RUN \
    K_VIM=$APP_USER_HOME/k-vim \
    && git clone https://gitee.com/huaixwang/k-vim.git $K_VIM \
    && WORKDIR $K_VIM \
    && sh -x install.sh \
    && WORKDIR $APP_USER_HOME

# install oh-my-zsh
RUN wget -qO- https://gitee.com/huaixwang/oh-my-zsh/raw/master/tools/install.sh | zsh || true
